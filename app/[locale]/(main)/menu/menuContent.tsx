    ...
    <section className="py-21 relative">
      ...
      <h2 className="text-5xl font-bold">{t("title")}</h2>
       <ul className="menu-nav bg-white/40 flex items-center center gap-2 py-3 px-5 rounded-full mt-14">
        {categories.map(item => <button onClick={()=> setCategoryId(item.id)} className={`text-lg px-5 py-1 font-semibold cursor-pointer ${item.isActive && "bg-white/40  rounded-full"}`} key={item.id}>{item.name}</button>)}
       </ul>
